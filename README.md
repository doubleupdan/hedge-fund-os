...
**Phase 1 complete:** repo scaffolded, Postgres schema live (accounts,
account_snapshots, risk_limits, risk_violations, proposed_trades, trades,
research_notes, decisions, trading_groups), risk validation gate written.
See `/docs/decision-log` for what's been decided and
`/docs/decision-log/OPEN-QUESTIONS.md` for what's still pending founder
input (broker, market-data vendor, capital amount).

**Now in progress:** proving the risk gate against a real test trade
(Step 7), then bringing in real strategy content from Average Joe Trading
Group's existing materials (Step 8+) — including a `strategies` table to
serve as the strategy registry, and wiring the correlation-exposure check
that `scripts/risk/validate_trade.py` currently stubs out.
