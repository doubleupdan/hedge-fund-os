...
exe's icon/version resources.

**Preferred fix — enable Developer Mode**, then use the normal build:
...
Developer Mode grants symlink creation to a standard account, which is all that
was missing. Running the build from an Administrator PowerShell works too.

**Fallback if you can't do either:**

```powershell
npm run dist:win:noadmin
```

That passes `-c.win.signAndEditExecutable=false`, skipping the winCodeSign
download entirely. It builds a working installer — but because it also skips
`rcedit`, **the exe keeps the default Electron icon** rather than the APEX OS
one, and gets no version metadata. The installer and Start-menu shortcut still
carry the right icon; it's the executable itself that misses out. Prefer
Developer Mode now that there's a real icon to embed.
...
apex-os/
  ...
  build/
    icon.png       1024px app icon (Linux + macOS source)
    icon.ico       16-256px, Windows
  scripts/
    vendor.mjs     bundles supabase-js, copies the font
    make-icon.mjs  generates both icons from code
    smoke.js       headless boot test
```

### About the icon

`build/icon.*` is generated by `npm run icon`, not drawn by hand. The mark is
the same "spark" glyph the UI already uses on the CEO node and every agent card,
built from the same 24×24 coordinates as the inline SVGs so the two can't drift
apart. `scripts/make-icon.mjs` writes the PNG and ICO directly (zlib is built
into Node) — no image library required.

Unlike `renderer/vendor/`, the icons **are** committed. They're small first-party
design assets that must exist for `npm run dist` to work straight after a clone,
whereas `vendor/` is regenerated third-party code. Re-run `npm run icon` after
changing any geometry constant.
