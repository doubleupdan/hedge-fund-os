...
- `007_trendline_strategy_from_presentation.sql` through
  `010_trendline_strategy_correction_v4.sql` — four successive correction
  passes on the AJTG Trendline Trading Strategy's entry/exit rules, moving
  from an EA-derived summary to the actual presentation deck and Trading
  Journal/Checklist document as primary sources. Each pass is kept as its
  own file rather than squashed, so the correction history stays visible.
  `010` also adds `trade_journal_entries`, a reflective/discipline record
  distinct from the transactional `trades` table.
